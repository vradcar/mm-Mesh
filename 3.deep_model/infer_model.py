...
if __name__=='__main__':
    m=mmwave()
    m.load_model('batch10000') # param is the last made batch model file (initially 400000)
    ...
    while not m.dataset.q_test.empty():
        m.dataset.q_test.get()
    m.dataset.task_next.join()
    m.dataset.task_test.join()
    for i in range(m.dataset.next_extra_process_size):
        m.dataset.task_list[i].join()
    # Addition [close]

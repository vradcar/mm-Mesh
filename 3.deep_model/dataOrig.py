...
class data():
    def __init__(self, batch_size, batch_length, pc_size):
        ...
        Tfile_path_trans = os.path.join(current_dir, '..', 'data', 'train', 'trans.mcl.3p8a.dat')
        Tfile_path_pquat = os.path.join(current_dir, '..', 'data', 'train', 'pmat.mcl.3p8a.dat')
        Tfile_path_betas = os.path.join(current_dir, '..', 'data', 'train', 'betas.mcl.3p8a.dat')
        Tfile_path_gender = os.path.join(current_dir, '..', 'data', 'train', 'gender.mcl.3p8a.dat')
        Tfile_path_pc = os.path.join(current_dir, '..', 'data', 'train', 'pc.mcl.3p8a.dat')

        self.pquat_sample_list=[0,1,2,4,5,16,17,18,19]
        self.trans=np.load(Tfile_path_trans).reshape((3, 8, 2400, 3)) # ((3, 8, 3000, 3))
        self.pquat=np.load(Tfile_path_pquat)[...,self.pquat_sample_list,:,:].reshape((3, 8, 2400, 9, 3, 3))
        self.betas=np.load(Tfile_path_betas) #(20,10)
        self.gender=np.load(Tfile_path_gender) #(20,1)
        self.pc=pk.load(open(Tfile_path_pc, 'rb'))

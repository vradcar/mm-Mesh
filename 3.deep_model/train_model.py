...
V_NO='mmMesh'
os.system('mkdir %s'%(V_NO)) # "Error": A subdirectory or file mmMesh already exists.
os.system('mkdir "%s/model"'%(V_NO)) # Error: The syntax of the command is incorrect.
...
class mmwave():
    def __init__(self):
        ...
        self.batch_size=32
        self.batch_rate=1
        self.train_size= 10000   # 400000 old valew RA changed it <-------------------------------------- [change to increase num of saved models]
        # self.train_length=64 # RA commented and added below line with value 4
        self.train_length=4  # changed from 64 to 4
        self.lr=0.001
        self.gpu_id=0
        ...
        if torch.cuda.is_available():
            ...
            self.device='cpu'
        self.pc_size=128 # Initially 128

        #print("calling data.py")
        # print("calling data.py with param [", self.batch_size, "] [", self.train_length, "] [", self.pc_size "]")
        ...
        # self.name_size=self.dataset.name_size # Commented for Multithread
        self.act_size=8
        self.vertice_rate=0.001
        self.betas_rate=0.1
        # self.test_length_size=self.dataset.test_length_size  # Commented for Multithread
        self.test_seg=1
        # self.test_length_seg=self.test_length_size//self.test_seg  # Commented for Multithread
    ...
    def cal_vs_from_qtbg(self, pquat_tensor, trans_tensor, betas_tensor, gender_tensor, b_size, l_size):
        with torch.no_grad():
            ...
            # if gender_tensor.size() == torch.Size([32, 64, 1]):
                # print("gender_tensor [shpe] |", gender_tensor.shape)
                # print("gender_tensor        |", gender_tensor[:,0,0])
                # print("gender_tensor [comp] |", gender_tensor[:,0,0]>0.5)
                
                # male_flag=gender_tensor[:,0,0]>0.5
                # female_flag=gender_tensor[:,0,0]<0.5

                # print("male flag |", male_flag)
                # print("feme flag |", female_flag)
            ...
            #pulled out of my if statement above:
            print("gender_tensor [shpe] |", gender_tensor.shape)
            print("gender_tensor        |", gender_tensor[:,0,0])
            print("gender_tensor [comp] |", gender_tensor[:,0,0]>0.5)
    ...
    def loss_model(self):
        ...
        with torch.no_grad():
            ...
            np_pc=self.dataset.get_test_pc() #Related to IndexError: tuple index out of range
    ...
    def train_model(self):
        ...
        print ("In Train Model before Loop")
        ...
        for i in range(self.train_size):
            ...
            if (i+1)%self.write_slot==0:
                print ("In Train Model Loop - 1 If")
            ...
            if (i+1) < 1000 and (i+1)%100==0:
                print ("In Train Model Loop - 2 If")
...
if __name__=='__main__':
    ...
    m.dataset=mmwave_data.data(m.batch_size, m.train_length, m.pc_size)
    m.name_size=m.dataset.name_size
    m.test_length_size=m.dataset.test_length_size
    m.test_length_seg=m.test_length_size//m.test_seg
    
    m.dataset.q_next_batch=multiprocessing.Manager().Queue()
    m.dataset.q_test=multiprocessing.Manager().Queue()
    m.dataset.v_flag=multiprocessing.Manager().Value('b', True)

    task_test=multiprocessing.Process(target=m.dataset.maintain_test_pc)
    task_next=multiprocessing.Process(target=m.dataset.maintain_next_batch, args=(True,))
    task_list=[]
    for i in range(m.dataset.next_extra_process_size):
        task_list.append(multiprocessing.Process(target=m.dataset.maintain_next_batch, args=(False,)))

    task_next.start() # if i added "m.dataset" in front of everything it would work. But i would get issues with no start method
    task_test.start()
    for i in range(m.dataset.next_extra_process_size):
        task_list[i].start()
...
# Code added - begin
    ...
    m.dataset.v_flag.value=False
    print('set to close', m.dataset.v_flag)
    time.sleep(10)
    while not m.dataset.q_next_batch.empty():
        m.dataset.q_next_batch.get()
    while not m.q_test.empty():
        m.dataset.q_test.get()
        task_next.join()
        task_test.join()
    for i in range(m.dataset.next_extra_process_size):
        task_list[i].join()

...
class data():
    def __init__(self, batch_size, batch_length, pc_size):
        #print("init() of data reached")

        self.batch_size=batch_size
        self.batch_length=batch_length
        self.pc_size=pc_size

        #print("pre current_dir")

        # Added for traversing file paths [Evaluation data paths]
        current_dir = os.path.dirname(__file__)
        Efile_path_trans = os.path.join(current_dir, '..', 'data', 'train', 'trans.mcl.3p8a.dat')
        Efile_path_pquat = os.path.join(current_dir, '..', 'data', 'train', 'pmat.mcl.3p8a.dat')
        Efile_path_betas = os.path.join(current_dir, '..', 'data', 'train', 'betas.mcl.3p8a.dat')
        Efile_path_gender = os.path.join(current_dir, '..', 'data', 'train', 'gender.mcl.3p8a.dat')
        Efile_path_pc = os.path.join(current_dir, '..', 'data', 'train', 'pc.mcl.3p8a.dat')

        self.pquat_sample_list=[0,1,2,4,5,16,17,18,19]
        self.trans=np.load(Efile_path_trans).reshape((3, 8, 2400, 3)) # ((3, 8, 3000, 3))
        self.pquat=np.load(Efile_path_pquat)[...,self.pquat_sample_list,:,:].reshape((3, 8, 2400, 9, 3, 3))
        self.betas=np.load(Efile_path_betas) #(20,10)
        self.gender=np.load(Efile_path_gender) #(20,1)
        self.pc=pk.load(open(Efile_path_pc, 'rb'))
        ...
        self.name_size=self.trans.shape[0] #8  ( it used to be 20 but with our data it is 3)
        self.act_size=self.trans.shape[1] #7 ( it is 8 with our data also)
        self.total_length_size=self.trans.shape[2] #3000 ( it is 2400 with our data also)
        # self.train_length_size=2400 # commented and changed this number in next line as we 3 8 2400 instead of 20 8 3000
        self.train_length_size=1800
        self.test_length_size=self.total_length_size-self.train_length_size
        # print ("self.total_length_size - ", self.total_length_size)
        # print ("test_length_size - ", self.test_length_size)
       ...
       # self.next_extra_process_size=16 # commented this and set it to zero below RA
        self.next_extra_process_size=0
        ...
        # self.task_test.start() # Error: TypeError: cannot pickle 'weakref.ReferenceType' object
        # for i in range(self.next_extra_process_size):
    ...
    def maintain_test_pc(self):
        while self.v_flag.value:
         #  print ("test Q size -",self.q_test.qsize())
...
# below code is commented in data as it is called from train

    # d.q_next_batch=multiprocessing.Manager().Queue()
    # d.q_test=multiprocessing.Manager().Queue()
    # d.v_flag=multiprocessing.Manager().Value('b', True)
    
    # task_test=multiprocessing.Process(target=d.maintain_test_pc)
    # task_next=multiprocessing.Process(target=d.maintain_next_batch, args=(True,))
    # task_list=[]
    # for i in range(d.next_extra_process_size):
    #     task_list.append(multiprocessing.Process(target=d.maintain_next_batch, args=(False,)))
...
# below code is commented in data as it is called from train
    ...
    # d.v_flag.value=False
    # print('set to close', d.v_flag)
    # time.sleep(10)
    # while not d.q_next_batch.empty():
    #     d.q_next_batch.get()
    # while not d.q_test.empty():
    #     d.q_test.get()
    #     task_next.join()
    #     task_test.join()
    # for i in range(d.next_extra_process_size):

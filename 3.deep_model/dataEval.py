...
class data():
    def __init__(self, batch_size, batch_length, pc_size):
        ...
        Efile_path_trans = os.path.join(current_dir, '..', 'data', 'eval', 'trans.mcl.3p8a.dat')
        Efile_path_pquat = os.path.join(current_dir, '..', 'data', 'eval', 'pmat.mcl.3p8a.dat')
        Efile_path_betas = os.path.join(current_dir, '..', 'data', 'eval', 'betas.mcl.3p8a.dat')
        Efile_path_gender = os.path.join(current_dir, '..', 'data', 'eval', 'gender.mcl.3p8a.dat')
        Efile_path_pc = os.path.join(current_dir, '..', 'data', 'eval', 'pc.mcl.3p8a.dat')

        self.pquat_sample_list=[0,1,2,4,5,16,17,18,19]
        self.trans=np.load(Efile_path_trans).reshape((3, 8, 600, 3)) # ((3, 8, 3000, 3))
        self.pquat=np.load(Efile_path_pquat)[...,self.pquat_sample_list,:,:].reshape((3, 8, 600, 9, 3, 3))
